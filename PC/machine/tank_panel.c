...
//
// Created on:  2014/7/24 at と 04:12:52 by wang fang.
...
*/	
	if(tid == TANK_02)
	{
		...
		SetCtrlVal(panel,PANEL_TANK_AC1_DOS_LAB, "H2O2干睪");
		...
		SetCtrlVal(panel,PANEL_TANK_AC2_DOS_LAB, "KOH干睪");
		...
		SetCtrlVal(panel,PANEL_TANK_AC3_DOS_LAB, "DIW干睪");
	...
	if(tid == TANK_04 || tid == TANK_07)
	{
		...
		SetCtrlVal(panel,PANEL_TANK_AC1_DOS_LAB, "IPA干睪");
		...
		SetCtrlVal(panel,PANEL_TANK_AC4_DOS_LAB, "ADD干睪");
	...
	else if(tid == TANK_11)
	{
		...
		SetCtrlVal(panel,PANEL_TANK_AC2_DOS_LAB, "HF干睪");
		...
		SetCtrlVal(panel,PANEL_TANK_AC1_DOS_LAB, "HCL干睪");
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (control) {
		...
		case PANEL_TANK_MMAKE_OFF:
			if(testBit(sys->tk[curTankId].hsMakeEnd,RB) == 0)
			{
				MessagePopup("巨兵ンぃ骸ì","皌佳ゼЧΘぃ睰");
				break;
			}
			if(g_setupMMake == 0)
			{
		...
		case PANEL_TANK_DOWN_OFF:
			if(testBit(sys->tk[curTankId].hsMakeEnd,RB) == 0)
			{
				MessagePopup("巨兵ンぃ骸ì","皌佳ゼЧΘぃ逼");
				break;
			}
